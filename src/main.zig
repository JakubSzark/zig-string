...
const String = struct {
    ...
    pub fn insertStr(self: *String, literal: []const u8, index: usize) StringError!void {
        if (self.buffer) |buffer| {
            ...
            if (self.len + literal.len > buffer.len) {
                try self.setCapacity((self.len * 2) + literal.len);
            }

            var i: usize = 0;
            while (i < literal.len) : (i += 1) {
                try self.insert(literal[i], index + i);
            }
        }
    }
    ...
    /// Replaces a character at the specified index
    // pub fn replace(self: *String, character: u8, index: usize) void {
